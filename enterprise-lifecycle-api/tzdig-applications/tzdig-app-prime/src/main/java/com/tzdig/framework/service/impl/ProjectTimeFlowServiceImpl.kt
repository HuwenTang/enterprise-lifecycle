...
class ProjectTimeFlowServiceImpl(
    ...
    private fun getProjectTimeFlow(
        ...
        val stepName = when (step) {
            ...
            PROJECT_REVIEW -> "签约核定·部门审核"
            PROJECT_REVIEW_ZB -> "签约核定·专班审核"
            PROJECT_START_REVIEW -> "开工认定"
            PROJECT_COMPLETION_REVIEW -> "竣工认定"
//            PROJECT_START_REVIEW_ZB -> "开工专班审核"
        ...
        if (list.isNotEmpty()) {
            ...
            val completed = when (step) {
                QUALITY_EVALUATION -> investment?.isQualityEvaluationComplete == true
    ...
    override fun getTimeFlow(id: String): List<ProjectTimeFlowVO> {
        ...
        result.addAll(getProjectTimeFlow(id, "签约", QUALITY_EVALUATION))
        result.addAll(getProjectTimeFlow(id, "签约", PROJECT_REVIEW))
        result.addAll(getProjectTimeFlow(id, "签约", PROJECT_REVIEW_ZB))
        // 注册
        ...
        // 备案
        val filingRecord = query<ProjectOnlineApproval> {
            ...
            select(ProjectOnlineApproval::applicationTime)
            and(ProjectDigitalInvestmentAttracting::id eq id)
            and(ProjectOnlineApproval::applicationTime.isNotNull)
            orderBy(ProjectOnlineApproval::applicationTime).desc()
        }.mapNotNull { it.applicationTime }
        if (filingRecord.isNotEmpty())
            result.add(
                ProjectTimeFlowVO(
                    isPrimary = true,
                    progress = "备案",
                    title = "申请备案",
                    time = filingRecord.first(),
                )
            )
        // 报批
        ...
        // 竣工
        result.addAll(getProjectTimeFlow(id, "竣工", PROJECT_COMPLETION_REVIEW))
        return result
    }
}
