...
export default function SigningApproval() {
  ...
  return (
    ...
    >
      <div style={{ background: "#fff", padding: "0.12rem 0.2rem 0.1rem" }}>
        <div className="list-page-subtitle">签约核定</div>
      ...
      <List key={listResetKey} style={{ height: "calc(100vh - 2.75rem)" }} finished={finished} onLoad={onLoad}>
        {finished && list.length === 0 ? (
          ...
          list.map((item, index) => {
          const zsid =
            (item as ProjectApprovalVo & { projectCode?: string }).projectCode ?? item.id ?? "";
          return (
            <div
              ...
              onClick={() => {
                navigate(`/qualitative-signing?id=${item.id}&isCom=0&zsid=${encodeURIComponent(zsid)}`);
              }}
            >
              <div
                style={{
              ...
              {item.signedDate ? (
                ...
                >
                  签约日期：<span style={{ color: "#8587fa" }}>{item.signedDate}</span>
                </div>
              ) : null}
