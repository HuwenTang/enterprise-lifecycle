...
export default function GlobalProject1(e:{year:string}) {
    const {year} = e
    ...
    const getQyxmYdqsTjxxViews = async () => {
        const data = await primeApi.getQyxmYdqsTjxxViews({year:+year})
    ...
    useEffect(() => {
        ...
        primeApi.getQyxmTotalViews({year:+year,month:+year1.split('月')[0]}).then(res => {
            data = res
        }).then(() => {
            primeApi.getQyxmTotalViews({year:+year-1,month:+year1.split('月')[0]}).then(res => {
                data1 = res
