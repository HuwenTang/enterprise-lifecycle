...
export default function ApproveDetail3(){
    ...
    const progressCount = async () => {
        const data = await primeApi.progressCount({onlineApprovalId: id?id:''})
                            ...
                            return (
                                <div key={index} onClick={() => {
                                    if (item.approvalStatus.includes('已办结')) {
                                        navigator(`/ApproveProcess2?count=${count}&total=${total}&id=${id}&pid=${item.id}`)
                                    }
                                }} style={{
                                    backgroundColor: '#fff',
                                    padding: '0.1rem',
                                    ...
                                    <div style={{
                                        ...
                                        color: '#333',
                                        fontSize: '0.12rem'
                                    }}>实施主体：{item.implementingSubject}</div>
                                    <div style={{
                                        marginTop: '0.1rem',
                                        color: '#333',
                                        fontSize: '0.12rem'
                                    }}>承办部门：{item.undertakingDepartment}</div>
                                </div>
                            )
                        })
                    }
