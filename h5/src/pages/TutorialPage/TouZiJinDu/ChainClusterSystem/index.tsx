...
const ChainClusterSystem: React.FC = () => {
  ...
  const [currentDate, setCurrentDate] = useState(dayjs(initialRange.start));
  ...
  const getChartOption = (type:string,obj:any) => {
    const totalNum = dataSource&&dataSource.length>0?dataSource[0].projectCount:100
     return {
      xAxis: {
        show: false,
        type: "value",
  ...
  const onFinish = () => {
    setMoney2(money);
    const { start, end } = getStartEndFromUrlOrMonth(searchParams, currentDate);
    loadData({
      start,
      end,
      minAmount: money === 0 ? undefined : money === 1 ? 10000 : money === 5 ? 50000 : 100000,
      maxAmount: money === 0 ? undefined : money === 1 ? 50000 : money === 5 ? 100000 : undefined,
    });
  };
  ...
  const loadData = async (params: { start: Date; end: Date; minAmount?: number; maxAmount?: number }) => {
    setLoading(true);
    try {
    const res = await primeApi.getIndustrialChainDistribution(params);
    // 转换响应数据格式到TreeNodeData
      const transformData = (item: IndustrialChainDistributionTreeVo, parentKey: string = '', index: number = 0): any => {
        ...
        return {
          ...
          children: item.children&&item.children.length>0 ? item.children.map((child, childIndex) =>
    ...
    <div className={styles.container}>
      ...
      <div className={styles.tabContainer}>
        ...
        <Breadcrumb>
          ...
          <Breadcrumb.Item><span style={{ color: '#1A237E', fontWeight: 600 }}>链群体系</span></Breadcrumb.Item>
        </Breadcrumb>
      </div>
                ...
                <div className={styles.monthContent}>
                  <div className={styles.monthCon}>
                    ...
                    <DatetimePicker
                      ...
                      value={new Date(dayjs(currentDate).year(),dayjs(currentDate).month(),1)}
                      showToolbar={false}
...
export default ChainClusterSystem;