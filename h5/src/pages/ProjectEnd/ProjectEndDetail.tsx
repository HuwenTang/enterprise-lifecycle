...
import { Button, Modal, Input, message, Steps, Tag, List } from "antd";
import { primeApi } from "../../api.ts";
import dayjs from "dayjs";
import { ExtZsProjProjectSignedVo } from "../../apis/index.ts";
...
export default function ProjectEnd() {
  ...
  const check = async () => {
    const data = await primeApi.checkCompletion({ zsId: id || "" });
    setIshow(data.value);
    if (data.value) {
  ...
  const handleOk = () => {
    ...
    createProjectDigitalProjectReview("1", value, value1);
    setIshow(false);
        ...
        <div style={{ backgroundColor: "#fff", padding: "0.2rem" }}>
          ...
          <div>
            ...
            <div
              ...
              className="content"
            >
              {msg?.endDate ? dayjs(msg.endDate).format("YYYY年MM月DD日") : "-"}
            </div>
          </div>
          ...
          <div>
            ...
            <div
              ...
              className="content"
            >
              {msg?.jgzzcl && msg.jgzzcl.length > 0 ? (
                <List
                  size="small"
                  dataSource={msg.jgzzcl}
