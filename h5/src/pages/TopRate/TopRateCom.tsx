...
export default function TopRateCom() {
    const [list, setList] = useState<CheckRankBo[]>()
    const [yingshou] = useState(true)
    ...
    const getCompanyDeptRanking = useCallback(async () => {
        const data = await primeApi.getCompanyDeptRanking({year: +year, company: name!})
        console.log(data)
        setList(data)
    }, [year, name])
    useEffect(() => {
        getCompanyDeptRanking()
    }, [getCompanyDeptRanking, yingshou]);
    return (
        <div style={{
            width:'100%',
            height:'100vh',
