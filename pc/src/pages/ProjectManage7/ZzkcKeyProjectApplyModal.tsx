...
export interface ZzkcKeyProjectApplyModalProps {
  open: boolean;
  onCancel: () => void;
  initialRecord?: any;
  onSuccess?: () => void;
...
/** 增资扩产「申报重点项目」专用弹窗：展示字段名称采用对照表「增资扩产」列（最左侧）用语 */
const ZzkcKeyProjectApplyModal = ({ open, onCancel, initialRecord, onSuccess }: ZzkcKeyProjectApplyModalProps) => {
  ...
  const resolveIndustryDictCatalog = () => {
    const signingTime =
      initialRecord?.signingTime ?? initialRecord?.approvalDate ?? initialRecord?.applicationTime;
    const year = signingTime ? dayjs(signingTime as any).year() : NaN;
    // 2026年及以后使用新字典
  ...
  const getIndustryCategoryOptions = async () => {
    try {
      ...
      console.error('获取行业分类（国民经济）选项失败:', error);
  ...
  useEffect(() => {
    ...
    const ptd = initialRecord.projectTypeDisplay ?? initialRecord.investmentType;
    const prefill: Partial<AddFormType> = {
      ...
      ...(ptd != null && String(ptd).trim() !== '' ? { projectTypeDisplay: String(ptd).trim() } : {}),
      ...(x813 ? { x: x813 } : {}),
      ...(ggIndustry ? { industryCode: ggIndustry } : {}),
    };

    const mainProductsRaw = initialRecord.mainProducts ?? initialRecord.mainProductsCapacity;
    if (mainProductsRaw != null && String(mainProductsRaw).trim() !== '') {
      prefill.mainProductsAndCapacity = String(mainProductsRaw).trim();
    }

    if (initialRecord.cityDistrict && areaData.length > 0) {
      const districts = areaData[0]?.children ?? [];
      const picked = districts.find((x: any) => x.value === initialRecord.cityDistrict);
      if (picked) {
        prefill.district = picked.value;
        const parks = picked.children ?? [];
        const parkOpts = parks.map((p: any) => ({ value: p.value, label: p.label }));
        setParkOptions(parkOpts);
        if (initialRecord.park != null && String(initialRecord.park).trim() !== '') {
          const pv = String(initialRecord.park).trim();
          const found = parks.find((p: any) => p.value === pv || String(p.label ?? '').trim() === pv);
          prefill.park = (found?.value ?? pv) as string;
        }
      }
    } else if (initialRecord.parkName && areaData.length > 0) {
      const districts = areaData[0]?.children ?? [];
  ...
  const buildZzkcKeyProjectApplyPayload = (values: AddFormType): any => {
    ...
    try {
      ...
      await primeApi.createProjectKeyProject({
        projectKeyProjectDto: {
          ...buildZzkcKeyProjectApplyPayload(storeAll),
  ...
  const onFinish: FormProps<AddFormType>['onFinish'] = async (values) => {
    ...
    try {
      await primeApi.createProjectKeyProject({
        projectKeyProjectDto: {
          ...buildZzkcKeyProjectApplyPayload(values),
  ...
  return (
    <Modal
      title={<div style={{ textAlign: 'center' }}>市级重点项目申报表（增资扩产）</div>}
      open={open}
    ...
    >
      ...
      >
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }}>
            ...
            />
            <div style={{ fontSize: '16px', fontWeight: 'bolder' }}>项目基本信息</div>
          </div>
          <Row gutter={16}>
            <Col span={12}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <Input disabled placeholder="与增资扩产备案项目代码一致" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item<AddFormType>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item<AddFormType> label="园区" name="park" rules={[{ required: true, message: '请选择园区' }]}>
                <Select
                  disabled
                  options={parkOptions}
                  placeholder="请先选择市区"
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item<AddFormType> label="项目类型" name="industryCategoryDisplay" rules={[{ required: true, message: '请选择项目类型' }]}>
                ...
                {({ getFieldValue }) =>
                  getFieldValue('industryCategoryDisplay') === '工业' ? (
                    <Form.Item<AddFormType>
                      label="产业方向"
                      name="x"
                      rules={[{ required: true, message: '请选择产业方向' }]}
                    >
                      <TreeSelect
                        placeholder="请选择"
                        allowClear
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item<AddFormType>
                ...
                rules={[{ required: true, message: '请选择内资或外资' }]}
              >
                <Select placeholder="请选择内资或外资" allowClear>
                ...
                {({ getFieldValue }) => {
                  ...
                  return (
                    ...
                    >
                      <InputNumber
                        style={{ width: '100%' }}
                        placeholder={unit ? `填写（${unit}）` : '请先选择内资或外资'}
                        min={0}
                        precision={2}
                      />
                    </Form.Item>
          ...
          <Row gutter={16}>
            <Col span={24}>
              ...
              >
                <Input.TextArea rows={3} placeholder="请输入主要产品、产能及主要建设内容" />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <Select placeholder="请选择投资类型（增资扩产等）" allowClear>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item<AddFormType>
                ...
                rules={[{ required: true, message: '请输入法人单位统一社会信用代码' }]}
              >
                <Input placeholder="请输入法人单位统一社会信用代码" />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
...
export default ZzkcKeyProjectApplyModal;
