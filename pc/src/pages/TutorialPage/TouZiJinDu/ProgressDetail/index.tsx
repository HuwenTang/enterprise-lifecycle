...
// 饼图组件 - 项目数量及占比
const ProgressProjectPieChart = (e: { data1: any }) => {
  const { data1 } = e;
      ...
      series: [
        {
          ...
          data: data1.length > 0
            ? data1.map((item: any) => ({
                ...
                value: item.accumulation?.count || 0,
              }))
            : [],
        },
      ],
...
// 柱状图组件 - 内外资项目对比
const ProgressDomesticForeignChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
      ...
      series: [
        {
          ...
          type: 'bar',
          data: data1.length > 0
            ? data1.map((item: any) => item.domestic?.count || 0)
            : [],
        },
        {
          name: '外资项目',
          type: 'bar',
          data: data1.length > 0
            ? data1.map((item: any) => item.foreign?.count || 0)
            : [],
        },
      ],
    };
...
// 折线图组件 - 投资额趋势
const ProgressInvestmentTrendChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
      ...
      series: [
        {
          ...
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.increasement?.amount || 0) / 10000).toFixed(2))
            : [],
        },
        {
          name: '本年累计投资',
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.accumulation?.amount || 0) / 10000).toFixed(2))
            : [],
        },
        {
          name: '内资投资',
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.domestic?.amount || 0) / 10000).toFixed(2))
            : [],
        },
      ],
    };
...
const InvestmentDetail: React.FC = () => {
  ...
  // 权限相关 state
  const [hasLevel2Permission, setHasLevel2Permission] = useState<boolean>(false); // level=2 是否有权限
  const [level3Areas, setLevel3Areas] = useState<string[]>([]); // level=3 的 areas 的 name 列表，用于匹配 districtName
  const [level4Areas, setLevel4Areas] = useState<string[]>([]); // level=4 的 areas 的 name 列表，用于匹配 parkName
  ...
  const fetchUserAreaPermissions = async () => {
    try {
      ...
      const level3AreasData = level3Data?.areas || [];
      const level4AreasData = level4Data?.areas || [];
      ...
      // level=3 时，areas 中有数据则用 name 和列表中 record.districtName 匹配
      if (level3AreasData.length > 0) {
        const level3AreaNames = level3AreasData
          .map((area) => area?.name)
          .filter((name): name is string => !!name)
          .map((name) => name.trim()); // 去除前后空格
        setLevel3Areas(level3AreaNames);
      } else {
        setLevel3Areas([]);
      }

      // level=4 时，areas 中有数据则用 name 和列表中 record.parkName 匹配
      // level=3 即使有数据也需要查询 level=4
      if (level4AreasData.length > 0) {
        const level4AreaNames = level4AreasData
          .map((area) => area?.name)
          .filter((name): name is string => !!name)
          .map((name) => name.trim()); // 去除前后空格
        setLevel4Areas(level4AreaNames);
      } else {
        setLevel4Areas([]);
      }
    } catch (error) {
  ...
  // 判断是否可以访问记录
  // 移除展开行和主行的区别，level2和level3无前后逻辑关系，只要匹配到该行数字可以点击
  const canAccessRecord = (record: TreeNodeData) => {
    ...
    // level=2 时，areas 中有数据则列表全部数字可点击
    if (hasLevel2Permission) {
      return true;
    }

    // level=3 时，用 name 和 record.districtName 匹配，匹配到就可以点击
    if (districtName && level3Areas.length > 0) {
      if (level3Areas.includes(districtName)) {
        return true;
      }
    }

    // level=4 时，用 name 和 record.parkName 匹配，匹配到就可以点击
    if (parkName && level4Areas.length > 0) {
      if (level4Areas.includes(parkName)) {
        return true;
      }
    }
  ...
  const handleNumberClick = (record: TreeNodeData, fieldName: string) => {
    ...
    const params = new URLSearchParams();
      ...
      if (record.district && record.districtName !== '泰州市') {
      ...
      // 根据字段名确定参数
      if (fieldName === 'increasementCount') {
        // 9月新增个数：传year, month, district(city)
        params.set('month', month.toString());
      } else if (fieldName === 'accumulationCount') {
        // 当年累计新增个数：传year, district(city)
        // month参数不设置
      } else if (fieldName === 'domesticCount') {
        // 内资项目个数：传year, isDomestic=true, district(city)
        params.set('isDomestic', 'true');
      } else if (fieldName === 'foreignCount') {
        // 外资项目个数：传year, isForeign=true, district(city)
        params.set('isForeign', 'true');
      }
  ...
  // 获取在建项目数据，startDate/endDate 为月份范围，end 传月尾，使用 UTC 避免时区导致 11 月变 10 月
  const fetchApprovalData = async (startDate: Dayjs, endDate: Dayjs, minAmount?: number | null, maxAmount?: number | null, innovativeClusters?: string) => {
    ...
    try {
      ...
      const response = await primeApi.getUnderConstructionProjects(params);
      ...
      console.error('获取在建项目数据失败:', error);
      message.error('获取在建项目数据失败');
      setDataSource([]);
      setSummaryData(null);
    } finally {
      setLoading(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: ['increasement', 'count'],
          key: 'increasementCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.increasement?.count || 0) - (b.increasement?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            ...
            return (
              <span
                ...
                onClick={() => handleNumberClick(record, 'increasementCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['increasement', 'amount'],
          key: 'increasementAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.increasement?.amount || 0) - (b.increasement?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: ['accumulation', 'count'],
          key: 'accumulationCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.accumulation?.count || 0) - (b.accumulation?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'accumulationCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['accumulation', 'amount'],
          key: 'accumulationAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.accumulation?.amount || 0) - (b.accumulation?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: ['domestic', 'count'],
          key: 'domesticCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.domestic?.count || 0) - (b.domestic?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'domesticCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['domestic', 'amount'],
          key: 'domesticAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.domestic?.amount || 0) - (b.domestic?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: ['foreign', 'count'],
          key: 'foreignCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.foreign?.count || 0) - (b.foreign?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'foreignCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['foreign', 'amount'],
          key: 'foreignAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.foreign?.amount || 0) - (b.foreign?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
  ...
  return (
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <h1 className={styles.pageTitle}>在建项目</h1>
        ...
        {currentView === 'table' && (
          <div className={styles.projectsTable}>
            <div className={styles.tableHeader}>
              <div className={styles.tableTitle}>在建项目基本情况</div>
            </div>
            <Table
