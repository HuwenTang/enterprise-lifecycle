...
// 饼图组件 - 项目数量及占比
const Progress2PieChart = (e: { data1: any }) => {
  const { data1 } = e;
...
// 柱状图组件 - 投资额对比
const Progress2InvestmentBarChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
      ...
      series: [
        {
          name: '当年累计',
          type: 'bar',
          data: data1.map((item: any) => ((item.year?.amount || 0) / 10000).toFixed(2)),
        },
        {
          name: '入库投资',
          type: 'bar',
          data: data1.map((item: any) => ((item.statisticalInvestment?.amount || 0) / 10000).toFixed(2)),
        },
      ],
    };
    chart.setOption(option);
...
// 折线图组件 - 投资趋势
const Progress2TrendChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
...
const InvestmentDetail: React.FC = () => {
  ...
  // 处理科创项目按钮点击事件
  const handleScienceProjectClick = () => {
  ...
  const handleNumberClick = (record: TreeNodeData, fieldName: string) => {
    ...
    // 根据字段名确定参数
    if (fieldName === 'monthCount') {
      // 本月竣工项目个数：传year, month, city
      params.set('month', month.toString());
    } else if (fieldName === 'yearCount') {
      // 本年竣工项目总数个数：传year, city
      // month参数不设置
    }
  ...
  // 获取投资完成率数据，startDate/endDate 为月份范围，end 传月尾，使用 UTC 避免时区问题
  const fetchApprovalData = async (startDate: Dayjs, endDate: Dayjs, minAmount?: number | null, maxAmount?: number | null, innovativeClusters?: string) => {
    ...
    try {
      ...
      const response: any = await primeApi.getInvestmentCompletionRate(params);
      ...
      console.error('获取投资完成率数据失败:', error);
      message.error('获取投资完成率数据失败');
      setDataSource([]);
      setSummaryData(null);
    } finally {
      setLoading(false);
  ...
  const columns = [
      ...
      children: [
        {
          ...
          key: 'monthCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.month?.count || 0) - (b.month?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            ...
            return (
              <span
                ...
                onClick={() => handleNumberClick(record, 'monthCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['month', 'amount'],
          key: 'monthAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.month?.amount || 0) - (b.month?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
    ...
    {
      title: '本年竣工项目总数',
      key: 'yearlyCompletedTotal',
      children: [
        {
          title: '个数',
          dataIndex: ['year', 'count'],
          key: 'yearCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.year?.count || 0) - (b.year?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'yearCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>计划总投资<br/>（亿元）</>,
          dataIndex: ['year', 'amount'],
          key: 'yearAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.year?.amount || 0) - (b.year?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
    ...
    {
      title: '入库投资情况',
      key: 'statisticalInvestment',
      children: [
        {
          title: '个数',
          dataIndex: ['statisticalInvestment', 'count'],
          key: 'statisticalInvestmentCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.statisticalInvestment?.count || 0) - (b.statisticalInvestment?.count || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'statisticalInvestmentCount')}
              >
                {value}
              </span>
            );
        ...
        {
          title: <>实际完成投资<br/>（亿元）</>,
          dataIndex: ['statisticalInvestment', 'amount'],
          key: 'statisticalInvestmentAmount',
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.statisticalInvestment?.amount || 0) - (b.statisticalInvestment?.amount || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
    ...
    },
    {
      ...
      children: [
        {
          title: '项目入库比例（%）',
          dataIndex: 'statisticalCountRate',
          key: 'statisticalCountRate',
          width: 130,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.statisticalCountRate || 0) - (b.statisticalCountRate || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? text + '%' : '0.00%'}</strong>
            ) : (
              text ? text  + '%' : '0.00%'
            ),
        },
        {
          title: '列统投资比例（%）',
          dataIndex: 'statisticalAmountRate',
          key: 'statisticalAmountRate',
          width: 130,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.statisticalAmountRate || 0) - (b.statisticalAmountRate || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? text  + '%' : '0.00%'}</strong>
            ) : (
    ...
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <Breadcrumb className={styles.mb15} style={{ fontSize: '20px' }}>
          ...
          <Breadcrumb.Item><a style={{ height: 'auto' }} onClick={() => navigate(-1)}>投资进度</a></Breadcrumb.Item>
          <Breadcrumb.Item>投资完成率</Breadcrumb.Item>
        </Breadcrumb>

        <h1 className={styles.pageTitle}>投资完成率</h1>
        ...
        {currentView === 'table' && (
          <div className={styles.projectsTable}>
            <div className={styles.tableHeader}>
              <div className={styles.tableTitle}>投资完成率情况</div>
              <div className={styles.filterControls}>
                ...
                <Button
                  ...
                  type="primary"
                  onClick={handleScienceProjectClick}
                >
                  科创项目
                </Button>
              </div>
