...
// 获取当前年份的9月
// 当年当月：默认查询当前年月数据
const getCurrentYearMonth = () => dayjs();
// 本页默认查询 2025年9月（月份范围）
...
const InvestmentDetail: React.FC = () => {
  ...
  const handleNumberClick = (record: TreeNodeData, fieldName: string) => {
    ...
    // 创新集群参数：当创新集群不是"项目总数"时才传递innovativeCluster
    if (record.innovativeCluster && record.innovativeCluster !== '项目总数') {
  ...
  // 获取产业链分布数据，startDate/endDate 为月份范围，end 传月尾，使用 UTC 避免时区问题
  const fetchApprovalData = async (startDate: Dayjs, endDate: Dayjs, projectType?: number, minAmount?: number, maxAmount?: number) => {
    ...
    try {
      ...
      const response = await primeApi.getIndustrialChainDistribution({
        start: startUtc,
        end: endUtc,
        projectType,
        minAmount,
  ...
  return (
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <h1 className={styles.pageTitle}>链群体系分布</h1>
        ...
        <div className={styles.projectsTable}>
          <div className={styles.tableHeader}>
            <div className={styles.tableTitle}>“8+13+X”链群分布情况</div>
