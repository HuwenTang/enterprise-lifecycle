...
// 表单字段类型定义
type FieldType = {
  type: number; // 项目类型：1-重点，2-1亿元，3-10亿元，4-全部
  date?: string; // 日期筛选
...
function GlobalProjectChart2({ data1 }: { data1: ProjectData[] }) {
  ...
  useEffect(() => {
    ...
    const colors = ['#407DFC', '#8A7AF7']; // 图表颜色配置
 ...
 * 开工详情主组件
 * 功能：展示开工项目的统计数据和图表
 * 特性：支持项目类型筛选、日期筛选、表格排序、树形展开
 */
const StartWorkDetail: React.FC = () => {
  ...
  // 状态管理
  const [dataSource, setDataSource] = useState<ProjectFagaiKeyProjectsStatsMergedVo[]>([]); // 表格数据源
  const [selectedProjectType, setSelectedProjectType] = useState<number>(0); // 当前选择的项目类型，默认为0（全部）
  const [sortField, setSortField] = useState<string | null>(null); // 排序字段
  const [sortOrder, setSortOrder] = useState<'ascend' | 'descend' | null>(null); // 排序顺序
  const [expandedKeys, setExpandedKeys] = useState<React.Key[]>([]); // 表格展开状态
  const [selectedDate, setSelectedDate] = useState<string>(''); // 选中的月份
  ...
  const listProjectFagaiKeyProjectsStats = async (date?: string) => {
    try {
      // 使用primeApi的listProjectFagaiKeyProjectsStats接口获取开工项目数据
      const params: any = {
        status: 1, // 开工项目状态
        type: 1, // 重点项目类型
        isDistrict: true, // 按地区统计
      };
      // 只有当date存在时才添加到参数中
      if (date) {
        params.date = date;
  ...
  const onFinish = (values: FieldType) => {
    ...
    // 处理月份，转换为该月第一天的yyyy-mm-dd格式
    const date = values.date ? dayjs(values.date).format('YYYY-MM') + '-01' : '';
    setSelectedDate(date);
    // 调用数据获取方法，并传入表单中选择的type值、date和产业链
    getMergedProjectFagaiKeyProjectsStats(values.type, date, values.industryChain);
    // 图表数据始终使用重置时的接口数据，不随项目类型改变
    const defaultType = 2;
    listProjectFagaiKeyProjectsStats1(defaultType, date);
    listProjectFagaiKeyProjectsStats(date);
  };
  // 重置表单
  const onReset = () => {
    form.resetFields();
    ...
    const defaultType = 2; // 图表数据使用固定参数
    ...
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <Breadcrumb className={styles.mb15} style={{ fontSize: '20px' }}>
          ...
          <Breadcrumb.Item>项目开工</Breadcrumb.Item>
        </Breadcrumb>

        <h1 className={styles.pageTitle}>开工项目情况</h1>
        {/* <p className={styles.pageSubtitle}>展示开工项目数据统计情况</p> */}
            ...
            >
              <Row>
                ...
                >
                  <Button
                    type="primary"
                    onClick={() => navigate('/tutorial/ke-chuang-xiang-mu')}
                    style={{
        ...
        <div className={styles.projectsTable}>
          <div className={styles.tableHeader}>
            <div className={styles.tableTitle}>开工项目情况统计</div>
        ...
        <div className={styles.chartsSection}>
          ...
          <div className={styles.chartsContainer}>
            ...
            <div className={styles.chartCard} style={{ width: '100%' }}>
              <div className={styles.chartCardHeader}>
                <h3 className={styles.chartCardTitle}>各市（区）项目投资对比</h3>
              </div>
              <div className={styles.chartCardBody}>
...
export default StartWorkDetail;
