/**
 * 开工竣工详情页面 - 竣工详情
 * 功能：展示竣工项目的统计数据，包括表格和图表
 * 特性：支持项目类型筛选、日期筛选、数据可视化
 */
import React, {useEffect, useRef, useState} from 'react';
import zhCN from 'antd/locale/zh_CN';
import {Button, ConfigProvider, DatePicker, Form, message, Row, Select, Space, Table,Breadcrumb} from 'antd';
...
interface ProjectData {
  ...
  plannedInvestmentTotal1?: number; // 重点项目实际完成投资
  plannedInvestmentTotal2?: number; // 亿元项目实际完成投资
}
...
function GlobalProjectChart4({ zdData, yyData }: { zdData: ProjectData[], yyData: ProjectData[] }) {
  ...
  useEffect(() => {
    ...
    // 使用重点项目的实际完成投资（转换为亿元）
    const zdPlannedInvestmentTotal = zdData.map(item => (item.plannedInvestmentTotal || 0) / 10000);

    // 使用亿元项目的实际完成投资（转换为亿元）
    // 假设yyData与zdData有相同的城市顺序，直接映射；如果顺序不同，需要根据城市名匹配
    const yyPlannedInvestmentTotal = yyData.map(item => (item.plannedInvestmentTotal || 0) / 10000);
...
const EndWorkDetail: React.FC = () => {
  ...
  const getMergedProjectFagaiKeyProjectsStats = async (selectedType?: number, date?: string, industryChain?: string) => {
    try {
      ...
      const params: any = {
        status: 2, // 竣工项目状态
        type: selectedType || 2, // 使用传入的类型或默认为2
      };
      // 只有当date存在时才添加到参数中
      if (date) {
        params.date = date;
  ...
  const listProjectFagaiKeyProjectsStats = async (date?: string) => {
    try {
      // 使用primeApi的listProjectFagaiKeyProjectsStats接口获取竣工项目数据
      const params: any = {
        status: 2, // 竣工项目状态
        type: 1, // 使用传入的类型
        isDistrict: true,
  ...
  useEffect(() => {
    ...
    const defaultType = 2; // 图表数据使用固定参数，与StartWorkDetail保持一致
    ...
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <Breadcrumb className={styles.mb15} style={{ fontSize: '20px' }}>
          ...
          <Breadcrumb.Item>项目竣工</Breadcrumb.Item>
        </Breadcrumb>

        <h1 className={styles.pageTitle}>竣工项目情况</h1>
        {/* <p className={styles.pageSubtitle}>展示竣工项目数据统计情况</p> */}
        ...
        <div className={styles.projectsTable}>
          <div className={styles.tableHeader}>
            <div className={styles.tableTitle}>竣工项目情况统计</div>
        ...
        <div style={chartStyles.chartsSection}>
          <div style={chartStyles.chartsHeader}>
            <h2 style={chartStyles.chartsTitle}>数据可视化分析</h2>
          </div>

          <div style={chartStyles.chartsContainer}>
            <div style={{...chartStyles.chartCard, ...chartStyles.chartCardHover}}>
              <div style={chartStyles.chartCardHeader}>
                <h3 style={chartStyles.chartCardTitle}>各市（区）重点项目</h3>
              </div>
              <div style={chartStyles.chartCardBody}>
                <GlobalProjectChart2 data1={zdData}/>
              </div>
            </div>
            <div style={{...chartStyles.chartCard, ...chartStyles.chartCardHover}}>
              <div style={chartStyles.chartCardHeader}>
                <h3 style={chartStyles.chartCardTitle}>各市（区）亿元项目</h3>
              </div>
              <div style={chartStyles.chartCardBody}>
            ...
            <div style={{...chartStyles.chartCard, ...chartStyles.chartCardHover}}>
              <div style={chartStyles.chartCardHeader}>
                <h3 style={chartStyles.chartCardTitle}>重点项目与亿元项目投资对比</h3>
              </div>
              <div style={chartStyles.chartCardBody}>
