...
import { UseLandStateTreeVo } from '@/services/apis';
...
interface TreeNodeData {
  ...
  // 备案项目总数
  totalFiledProjects?: number;
  totalInvestment?: number;
  // 需新增用地项目
  newLandRequiredCount?: number;
  requiredArea?: number;
  // 已供土地
  landSuppliedCount?: number;
  suppliedArea?: number;
  // 无需新增用地项目
  noNewLandRequiredCount?: number;
  noNewLandInvestment?: number;
  // 其中：租用厂房项目
  leasedFactoryCount?: number;
  leasedFactoryInvestment?: number;
  districtName?: string;
  parkName?: string;
  key: React.Key;
  isSummary?: boolean;
...
// 饼图组件 - 备案项目数量及占比
const LandProjectPieChart = (e: { data1: any }) => {
  const { data1 } = e;
      ...
      series: [
        {
          ...
          data: data1.length > 0
            ? data1.map((item: any) => ({
                ...
                value: item.totalFiledProjects || 0,
              }))
            : [],
        },
      ],
...
// 柱状图组件 - 用地项目对比
const LandUseChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
      ...
      series: [
        {
          ...
          type: 'bar',
          data: data1.length > 0
            ? data1.map((item: any) => item.newLandRequiredCount || 0)
            : [],
        },
        {
          name: '已供土地项目',
          type: 'bar',
          data: data1.length > 0
            ? data1.map((item: any) => item.landSuppliedCount || 0)
            : [],
        },
      ],
    };
...
// 折线图组件 - 投资额趋势
const LandInvestmentTrendChart = (e: { data1: any }) => {
  const { data1 } = e;
  const chartRef = useRef(null);
      ...
      series: [
        {
          ...
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.totalInvestment || 0) / 10000).toFixed(2))
            : [],
        },
        {
          name: '无需新增用地投资',
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.noNewLandInvestment || 0) / 10000).toFixed(2))
            : [],
        },
        {
          name: '租用厂房投资',
          type: 'line',
          data: data1.length > 0
            ? data1.map((item: any) => ((item.leasedFactoryInvestment || 0) / 10000).toFixed(2))
            : [],
        },
      ],
    };
...
const LandDetail: React.FC = () => {
  ...
  const handleNumberClick = (record: TreeNodeData, fieldName: string) => {
    ...
    // 根据字段名确定参数
    if (fieldName === 'totalFiledProjects') {
      // 备案项目总数：传year, month
      // 不需要额外参数
    } else if (fieldName === 'newLandRequiredCount') {
      // 需新增用地项目：传year, month, isAddLand=true
      params.set('isAddLand', 'true');
    } else if (fieldName === 'landSuppliedCount') {
      // 已供土地：传year, month, isLand=true
      params.set('isLand', 'true');
    } else if (fieldName === 'noNewLandRequiredCount') {
      // 无需新增用地项目：传year, month, isAddLand=false
      params.set('isAddLand', 'false');
    } else if (fieldName === 'leasedFactoryCount') {
      // 租用厂房项目：传year, month, isFactory=true
      params.set('isFactory', 'true');
    }
  ...
  const fetchApprovalData = async (startDate: Dayjs, endDate: Dayjs, projectType?: string, minAmount?: number, maxAmount?: number) => {
    ...
    try {
      ...
      const response = await primeApi.getLandUse({
        start: startUtc,
        end: endUtc,
        projectType,
        minAmount,
      ...
      const transformData = (item: UseLandStateTreeVo, parentKey: string = '', index: number = 0): TreeNodeData => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      sorter: (a: TreeNodeData, b: TreeNodeData) => (a.totalFiledProjects || 0) - (b.totalFiledProjects || 0),
      render: (text: number, record: TreeNodeData) => {
        ...
        return (
          <span
            ...
            onClick={() => handleNumberClick(record, 'totalFiledProjects')}
          >
            {value}
          </span>
        );
      ...
      children: [
        {
          ...
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.newLandRequiredCount || 0) - (b.newLandRequiredCount || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'newLandRequiredCount')}
              >
                {value}
              </span>
            );
        ...
        },
        {
          ...
          key: 'requiredArea',
          width: 130,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.requiredArea || 0) - (b.requiredArea || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: 'landSuppliedCount',
          key: 'landSuppliedCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.landSuppliedCount || 0) - (b.landSuppliedCount || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'landSuppliedCount')}
              >
                {value}
              </span>
            );
        ...
        },
        {
          ...
          key: 'suppliedArea',
          width: 130,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.suppliedArea || 0) - (b.suppliedArea || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              "--"
              // <strong>{text ? text.toFixed(2) : '0.00'}</strong>
    ...
    {
      title: '无需新增用地项目',
      key: 'noNewLandRequired',
      children: [
        {
          title: '个数',
          dataIndex: 'noNewLandRequiredCount',
          key: 'noNewLandRequiredCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.noNewLandRequiredCount || 0) - (b.noNewLandRequiredCount || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'noNewLandRequiredCount')}
              >
                {value}
              </span>
            );
        ...
        },
        {
          ...
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.noNewLandInvestment || 0) - (b.noNewLandInvestment || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
    ...
    },
    {
      ...
      children: [
        {
          title: '个数',
          dataIndex: 'leasedFactoryCount',
          key: 'leasedFactoryCount',
          width: 100,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.leasedFactoryCount || 0) - (b.leasedFactoryCount || 0),
          render: (text: number, record: TreeNodeData) => {
            const value = text || 0;
            if (!canAccessRecord(record)) {
              return (
            ...
            return (
              <span
                className={styles.highlightNumber}
                style={record.isSummary ? { fontWeight: 'bold' } : undefined}
                onClick={() => handleNumberClick(record, 'leasedFactoryCount')}
              >
                {value}
              </span>
            );
        ...
        },
        {
          ...
          width: 150,
          sorter: (a: TreeNodeData, b: TreeNodeData) => (a.leasedFactoryInvestment || 0) - (b.leasedFactoryInvestment || 0),
          render: (text: number, record: TreeNodeData) =>
            record.isSummary ? (
              <strong>{text ? (text / 10000).toFixed(2) : '0.00'}</strong>
            ) : (
    ...
    <div className={styles.investmentDetailPage}>
      <div className={styles.container}>
        ...
        <Breadcrumb className={styles.mb15} style={{ fontSize: '20px' }}>
          ...
          <Breadcrumb.Item>用地保障</Breadcrumb.Item>
        </Breadcrumb>

        <h1 className={styles.pageTitle}>用地保障</h1>
        ...
        {currentView === 'table' && (
          <div className={styles.projectsTable}>
            <div className={styles.tableHeader}>
              <div className={styles.tableTitle}>备案项目用地情况</div>
            </div>
            <Table
            ...
            <div className={styles.chartsContainer}>
              <div className={styles.chartCard}>
                <div className={styles.chartCardHeader}>
                  <h3 className={styles.chartCardTitle}>备案项目数量及占比</h3>
                </div>
              ...
              <div className={styles.chartCard}>
                <div className={styles.chartCardHeader}>
                  <h3 className={styles.chartCardTitle}>各市区用地项目对比</h3>
                </div>
                <div className={styles.chartCardBody}>
                  <LandUseChart
                    data1={chartData}
